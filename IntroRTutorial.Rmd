...
## BEFORE YOU BEGIN
Use the **Chrome** browser for this tutorial.  If you are not in Chrome, copy the link and open it in Chrome.  If your screen is **grey** do the following: 

* press the following keys, holding them all down (you'll need to use four fingers for this!):  
**ctr + shift + alt + A**
* **deselect** all but the last Network Method and **click OK**  
* refresh your browser by pressing **F5**

![ctrshiftaltA](images/IntroR_ctraltshiftA.png){width="180"}   |  ![Select network method](images/IntroR_selnetmeth.png){width="300"} 

If you don't have Chrome, the tutorial will work in Internet Explorer but the **Next Topic** button does not work.  There is a workaround:  

* Click on **Next Topic**
* Press **F5** to refresh your browser.  

You will have to do this each time you reach the end of a topic.  
...
Throughout this tutorial you will be given opportunities to write code.  The **R** language has a very simple syntax with a only a few rules.  You won't need to remember this for the tutorial, but here is a summary of the rules: 

* **R** language is case sensitive: `banana` and `Banana` will be interpreted as different things. 

* Spaces and indentation carry no meaning (unlike other languages).  However, they are recommended for ease of reading.  

* All alphanumeric symbols are allowed.  

* Names (e.g. of objects) have few rules.  A name must start with a letter or `.`, and if it starts with `.` the second character must not be a digit). Names are effectively unlimited in length.

* Commands are separated by a new line, or a semi-colon (‘;’).  A single command can run over several lines.  Some commands can be chained together with special symbols (e.g. `+` or `%>%`) - not covered in this tutorial.  

* Comments can be put almost anywhere, starting with a hash `#`, everything to the end of the line is a comment.
...
* Tutorial 1: [R data types](#section-tutorial-1-r-data-types) (10 minutes)
* Tutorial 2: [Importing data](#section-tutorial-2-importing-data) (5 minutes)
* Tutorial 3: [Functions and operators](#section-tutorial-3-functions-and-operators) (20 minutes)
* Tutorial 4: [Summary statistics](#section-tutorial-4-statistics) (15 minutes)
* Tutorial 5: [Random distributions](#section-tutorial-5-random-distributions) (5 minutes)
* Tutorial 6: [Graphs](#section-tutorial-6-graphs) (10 minutes) 

Click **Next topic** to read on.  

## TUTORIAL 1: R DATA TYPES

This session should take about **10** minutes to complete.  It introduces the ways in which R handles data, namely: 
...
You have four [pitfall traps](http://en.wikipedia.org/wiki/Pitfall_trap) and have counted how many invertebrates of two species are found in each. You found the following numbers of individuals of each species in the traps:  
...
The `cbind` (**c**olumn **bind**) function combined the vectors into a matrix as columns.  You could also use `rbind` (**r**ow **bind**) to bind vectors as rows.  
...
###
You can do this by specifying the number of rows (12 months = 12 rows).  Click **Run Code** to see how it works: 

```{r weather.records,  exercise=TRUE, exercise.eval=FALSE}
matrix(temp, nrow= 12)
```

This creates a matrix with one row for each month and one column for each year.  Note that `R` has automatically worked out that there must be 5 columns.  
...
A data frame consists of two or more **vectors** of the **same length**.  Unlike a matrix each column can contain **different types of data**.  It is the preferred way of storing tabular data in R.  It is also the default way data from Excel spreadsheets is imported into **R**, and for this reason the most common way ecological data will be imported for analysis.
...
###
The `data.frame` function combines the vectors into a data frame.  It also automatically assigns a class to each column.  In the example above it has assigned the character data to a factor class, and the numeric data to class double (in R this is equivalent to numeric).  

Column *plants* is class **factor**  
Column *cover* is class **double** or **numeric**  

You could also use `cbind` and **R** would automatically create a `data.frame` because a matrix cannot contain vectors of different data types.
...
* *Correct*: `quadrat[1]` You can index a column by its number, without specifying any rows.  It returns a *data frame*.   
* *Correct*: `quadrat[,1]` You access values by specifying rows and columns: `[row , column]`.  Here we left the row index blank, which returns all rows, and the first column. It returns a *data frame*.    
* *Incorrect*: `quadrat.plants` **R** thinks this is an object called   `quadrat.plants`: we haven't defined such an object.   
* *Correct*: `quadrat$plants` The `$` character is used to reference a column name.  It returns a *vector*.
* *Correct*: `quadrat["plants"]` Here we have used a character string to reference a column by its name.  It returns a *data frame*.     
...
You want to analyse some vegetation height data.  It is saved in the following file in your working directory: `veg-heights.csv`.  
...
The **read.csv()** function can bring csv file data into **R**.  
...
`header = TRUE`  A **parameter** for the `read.csv()` function.  We will learn more about parameters later.  
...
The first argument `veg-heights.xlsx` is the name of the Excel file.  

Excel workbooks can consist of many worksheets, so we need to specify which one we want to import.  The second argument `1` references the worksheet that you wish to import, counting from the left.  

*Note*: `read.xlsx` does not support referencing worksheets by name.  
...
This session should take about **20** minutes to complete.  It explains:
...
By default, R operates on objects *element-wise*, i.e. on each element of the vector in turn.  It has multiplied each measurement by 100.  
...
**R** has again operated element-wise, but in this case with two vectors.  It has subtracted each element in one vector from the corresponding vector in the other.  

You can visualise it like this: 
```{r}
knitr::kable(
cbind(element = seq(1:12), 
      water.table.rpt,
      operator = rep("-", 12), 
      water.table, 
      result = water.table.rpt - water.table)
)
```
...
This session should take about **15** minutes to complete.  It gives a very brief introduction to calculating statistics from **R**
...
## TUTORIAL 5: RANDOM DISTRIBUTIONS

This session should take about **5** minutes to complete.  It will introduce a number of statistical distributions, the generations of random numbers and how to prepare simple plots and charts in **R**
...
###
`sample` draws random numbers from a sample.  Here we have drawn 10 unique numbers at random from a sample of whole numbers between  1 and 100, e.g. to select ten random survey points along  a 100m transect.
```{r echo=TRUE}
sample(x = 1:100, size = 10, replace = F)
```

`x = 1:100` specifies the population we are sampling from. 
`size = 10` specifies the number of samples we wish to take. 
`replace = F` specifies that once a number has been randomly selected, it cannot be chosen again (i.e. without replacement).
...
###
Do you think there is a relationship between vegetation height and cover here? We can add a simple regression line by combining the functions `lm` (to fit a linear model), and `abline` (literally a *line* from *a* to *b*). 

```{r echo=TRUE}
plot(formula = height ~ cover, data = veg.all)
abline(lm(formula = height ~ cover, data = veg.all), col = "red")
```

Note the *graphical parameter* `col` which changes the colour of the line to red. More on graphical parameters below.  
...
###
You see that the data is stacked in each year, which suggests that `year` is actually a categorical variable.  This means we can use boxplots. 
...
Count data is best presented as a bar plot.  Have a look at our deer count example from earlier.  The data is now in a  matrix showing the average number of deer counted in each year.  

```{r deerbar-setup, echo=F}
deer1 <- data.frame(red = c(5, 17, 28), roe = c(33, 35, 75))
deer2 <- data.frame(red = c(7, 15, 50), roe = c(20, 22, 61))

deer <- matrix(c(mean(deer1$red), mean(deer1$roe), mean(deer2$red), mean(deer2$roe)), 
               ncol = 2)
row.names(deer) = c("red", "roe")
colnames(deer) <- c("year1", "year2")
deer
```

The following command prints a simple bar plot:
```{r deerbar, echo=T}
barplot(deer)  
```

### Introduction to graphical parameters
At the moment, this chart lacks a legend.  We can use *parameters* to improve charts. Key parameters for bar plots are:  

* `legend` (adds a legend using row names)
* `beside` (unstacks bar plots) 
* `horiz` (flips bar plots by 90 degrees)
* `col` (adds colour)

###
You can see how they are used in this example: 

```{r deerbar2-setup, echo=F}
deer1 <- data.frame(red = c(5, 17, 28), roe = c(33, 35, 75))
deer2 <- data.frame(red = c(7, 15, 50), roe = c(20, 22, 61))
...
### Excercise: plot parameters
Try changing the following code to:

1. make the bar plot vertical and
2. change the blue bars to yellow.  
...
```{r deerex, exercise=TRUE, exercise.lines = 7}
barplot(deer, 
        legend = TRUE, 
        horiz = TRUE, 
        beside = TRUE, 
...
### 
There are a huge number of graphical parameters.  They can be added to any of the above plotting functions (`plot`, `boxplot`, `barplot`) and many others.  A useful introduction to common parameters can be found here: https://www.statmethods.net/advgraphs/parameters.html.  

To see a full list of parameters, see the helpfile `par` by running the code below: 

```{r parhelp, exercise=TRUE}
?par
```

Parameters can also be set for all plots.  This uses the `par` function which must be run **before** the plot is generated.  These parameters will then be used for all subsequent plots for the duration of the R session, or until they are changed.  

###
End of section **GRAPHS**

###
This is the end of the **Introduction to R Tutorial**.  
